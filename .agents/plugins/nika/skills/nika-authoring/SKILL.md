...
| `on_error:` | exactly ONE action — `recover:` · `skip:` (preserves the original error at `tasks.X.error`) — with an optional `on_codes:` filter · the default (no `on_error:`) IS failure, and there is no keyword for saying so |
...
spelling. This is a complete nine-key file (checked on 0.109.2 · PARSE
clean):
